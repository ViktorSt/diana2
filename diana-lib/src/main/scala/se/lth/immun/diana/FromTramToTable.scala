...
import akka.io.Tcp
import Tcp._
import se.lth.immun.diana.DataModel._
import se.lth.immun.diana.Filters._
import se.lth.immun.diana.DetectionAndGrouping._
import se.lth.immun.diana.PeakFinder._
...
object FromTramToTable {
  ...
  def startVSpeaks(dianaDatabase: Seq[DianaLib.Trace]) = {
    /* My database is ArrayBuffer[Trace] (where trace is in this class), Needed format is DianaLib.Trace instead. 
     * Conversion needed to start process, then my format results (ArrayBuffer[Peak] need to be converted to 
     * Seq[PCGroup] and returned.
     */
    startCalculating(dianaDatabase)
    
  }
  def startCalculating(dianaDatabase: Seq[DianaLib.Trace]){
...
//            val ids=dianaDatabase(i).channel.id
